<div class="front_page_section front_page_section_testimonials<?php
			$freightco_scheme = freightco_get_theme_option('front_page_testimonials_scheme');
			if (!freightco_is_inherit($freightco_scheme)) echo ' scheme_'.esc_attr($freightco_scheme);
			echo ' front_page_section_paddings_'.esc_attr(freightco_get_theme_option('front_page_testimonials_paddings'));
		?>"<?php
		$freightco_css = '';
		$freightco_bg_image = freightco_get_theme_option('front_page_testimonials_bg_image');
...
?>><?php
	// Add anchor
	$freightco_anchor_icon = freightco_get_theme_option('front_page_testimonials_anchor_icon');	
	$freightco_anchor_text = freightco_get_theme_option('front_page_testimonials_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_testimonials_inner<?php
			if (freightco_get_theme_option('front_page_testimonials_fullheight'))
				echo ' freightco-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$freightco_css = '';
			$freightco_bg_mask = freightco_get_theme_option('front_page_testimonials_bg_mask');
			$freightco_bg_color = freightco_get_theme_option('front_page_testimonials_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$freightco_caption = freightco_get_theme_option('front_page_testimonials_caption');
			if (!empty($freightco_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo !empty($freightco_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($freightco_caption); ?></h2><?php
			}
		
			// Description (text)
			$freightco_description = freightco_get_theme_option('front_page_testimonials_description');
			if (!empty($freightco_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo !empty($freightco_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($freightco_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_testimonials_output"><?php 
				if (is_active_sidebar('front_page_testimonials_widgets')) {
					...
					if (!freightco_exists_trx_addons())
						...
						freightco_customizer_need_widgets_message('front_page_testimonials_caption', 'ThemeREX Addons - Testimonials');
				}
			?></div>
		</div>
	</div>
