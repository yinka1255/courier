		...
		?>
			...
			<div class="options-control-field">
				<?php foreach ( $this->choices as $value => $params ): ?>
					...
					<label>
						<input type="radio" value="<?php echo esc_attr( $value ) ?>" name="op-options[<?php echo esc_attr( $this->id ) ?>]" <?php checked( $this->value(), $value ) ?> />
						<span data-tooltip="<?php echo esc_attr( $params['tooltip'] ) ?>">
							<img src="<?php echo esc_url( $params['src'] ) ?>" alt="<?php echo esc_attr( $value ) ?>" />
						</span>
					</label>
