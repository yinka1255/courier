...
?>><?php
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo (!empty($item['link']) && $args['type'] == 'default') ? ' sc_icons_item_linked' : ''; ?><?php
			if (!empty($item['image'])) {
				echo ' with_image_bg ';
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('relimg'), 'icons-default'));
					echo esc_attr(freightco_add_inline_css_class('background-image:url('.esc_url($item['image']).');'));
			} ?>"><?php
			...
			if (!empty($item['title']) || !empty($item['description'])) {
				?>
				<div class="sc_icons_item_container">
				<?php
			}
			if (!empty($item['title'])) {
			...
			if (!empty($item['link_title']) && !empty($item['link']) && $args['type'] != 'default') {
				?><a href="<?php echo esc_url($item['link']); ?>" class="sc_button sc_button_simple"><?php echo esc_html__($item['link_title']); ?></a><?php
			}
			if (!empty($item['title']) || !empty($item['description'])) {
				?>
				</div>
			...
			if (!empty($item['link']) && $args['type'] == 'default') {
