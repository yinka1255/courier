	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		while ( $query->have_posts() ) { $query->the_post();
			if ($args['slider'] && !empty($args['slider_pagination_thumbs'])) {
				...
				$name = get_the_title();
				$metasubtitle = get_post_meta(get_the_ID(), 'trx_addons_options', true);
				$subtitle = $metasubtitle['subtitle'];					
				$pagination_bullets .= '<span class="slider-pagination-button swiper-pagination-button"'
									. '><span class="custom-image-container"><span class="custom-image"'
									. ($img ? ' style="background-image: url('.esc_url($img).');"' : '')
									. '></span></span><span class="custom-title">'
									. '<span class="custom-title-name">'
									. esc_html($name)
									. '</span>'
									. '<span class="custom-title-subname">'
									.  esc_html($subtitle)
									. '</span>'
									. '</span></span>';
			}
