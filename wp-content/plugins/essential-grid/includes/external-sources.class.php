...
class Essential_Grid_Instagram {
    ...
    */
	 ...
	 */
	public function __construct($access_token,$transient_sec=86400) {
		$this->transient_sec = $transient_sec;
		$this->access_token = $access_token;
	}
	...
	public function get_public_photos($search_user_id,$count){
		...
		if(!empty($search_user_id) && !empty($this->access_token)){

			$url = 'https://api.instagram.com/v1/users/'.$search_user_id.'/media/recent/?&access_token='.$this->access_token.'&count='.$count;
			...
			$this->instagram_output_array($rsp->data,$count);
	...
	*/
	public function get_user_id($search_user_id) {
		$url = 'https://api.instagram.com/v1/users/search?q='.$search_user_id.'&access_token='.$this->access_token;
