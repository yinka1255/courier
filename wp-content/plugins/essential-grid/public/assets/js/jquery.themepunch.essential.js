...
(function(jQuery,undefined){
	...
	jQuery.fn.extend({
					...
					********************************/
					...
					jQuery('body').on('click', '.esgbox', function() {
						...
						if(opt.lightBoxMode === 'filterall') {
							$this.closest('.esg-grid').find('.isvisiblenow, .fitsinfilter').find('.esgbox').attr('rel', 'items-' + parseInt(Math.random() * 9999, 10));
						}
							...
							if(tpe === 'enter') {
								...
								switch(itm) {
									...
									case 'esg-item-fade':
										...
										// console.log(val);
										// console.log(Math.max(Math.min(val, 1), 0));
									break;
			...
			if(t && t.group && Array.isArray(t.group) && t.hasOwnProperty('index')) {
				...
				if(itm && itm.element) {
					...
					if(itm.length && itm.hasClass('esgbox-post') && itm.attr('href') !== '#eg-lightbox-post-content') {
						...
						var titl;
						if(t.helpers && t.helpers.title && t.helpers.title.type) titl = t.element.attr('data-posttitle') || t.element.attr('lgtitle');
						
						loadPostContent(itm, getOptions(jQuery(this)), false, t.index, titl);
						return false;
		...
		*/
		...
		function loadPostContent($this, opt, trigger, index, mainTitle) {
			...
			jQuery.ajax({
				...
				if(response.success) {
					...
					if(trigger) {
						...
						if(mainTitle) {
							var titlEl = jQuery('.esgbox-title div');
							if(titlEl.length) titlEl.text(mainTitle);
						}
